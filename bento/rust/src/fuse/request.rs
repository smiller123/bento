...
pub fn dispatch<T: FileSystem>(fs: &mut T, opcode: fuse_opcode, sb: RsSuperBlock, inarg: &bento_in, outarg: &mut bento_out) -> i32 {
    match opcode {
        ...
        fuse_opcode_FUSE_SETATTR => {
            ...
            fs.setattr(sb, &req, inarg.h.nodeid, mode, uid, gid, size, atime,
        ...
        fuse_opcode_FUSE_MKNOD => {
            ...
            let entry_out = unsafe { &mut *(outarg.args[0].value as *mut fuse_entry_out) };
            let mut reply = ReplyEntryInternal { reply: Ok(entry_out) };
            fs.mknod(sb, &req, inarg.h.nodeid, name, mknod_in.mode, mknod_in.rdev, &mut reply);
            match reply.reply() {
                Ok(_) => {
                    0
                },
        ...
        fuse_opcode_FUSE_RENAME | fuse_opcode_FUSE_RENAME2 => {
            ...
            let mut reply = ReplyEmptyInternal { reply: Err(-(ENOSYS as i32)) };
            fs.rename(sb, &req, inarg.h.nodeid, oldname, rename_in.newdir, newname, &mut reply);
            match reply.reply() {
                Ok(_) => {
                    0
                },
        ...
        fuse_opcode_FUSE_LINK => {
            ...
            let entry_out = unsafe { &mut *(outarg.args[0].value as *mut fuse_entry_out) };
            let mut reply = ReplyEntryInternal { reply: Ok(entry_out) };
            fs.link(sb, &req, link_in.oldnodeid, inarg.h.nodeid, name, &mut reply);
            match reply.reply() {
                Ok(_) => {
                    0
                },
        ...
        fuse_opcode_FUSE_READ => {
            ...
            let data_out = unsafe { &mut *(outarg.args[0].value as *mut MemContainer<raw::c_uchar>) };
            let mut reply = ReplyDataInternal { reply: Ok(data_out) };
            fs.read(sb, &req, inarg.h.nodeid, read_in.fh, read_in.offset as i64, read_in.size, &mut reply);
            match reply.reply() {
                Ok(buf) => {
                    buf.len() as i32
                },
        ...
        fuse_opcode_FUSE_WRITE => {
            ...
            fs.write(sb, &req, inarg.h.nodeid, write_in.fh, write_in.offset as i64, data,
        ...
        fuse_opcode_FUSE_FLUSH => {
            ...
            let mut reply = ReplyEmptyInternal { reply: Err(-(ENOSYS as i32)) };
            fs.flush(sb, &req, inarg.h.nodeid, flush_in.fh, flush_in.lock_owner, &mut reply);
            match reply.reply() {
                Ok(_) => {
                    0
                },
        ...
        fuse_opcode_FUSE_RELEASE => {
            ...
            let mut reply = ReplyEmptyInternal { reply: Err(-(ENOSYS as i32)) };
            fs.release(sb, &req, inarg.h.nodeid, release_in.fh, release_in.flags,
        ...
        fuse_opcode_FUSE_FSYNC => {
            ...
            fs.fsync(sb, &req, inarg.h.nodeid, fsync_in.fh, datasync,
                       &mut reply);
            match reply.reply() {
                Ok(_) => {
                    0
        ...
        fuse_opcode_FUSE_READDIR => {
            ...
            fs.readdir(sb, &req, inarg.h.nodeid, read_in.fh, read_in.offset as i64, &mut reply);
            match reply.reply() {
                Ok(buf) => {
        ...
        fuse_opcode_FUSE_RELEASEDIR => {
            ...
            let release_in = unsafe { &*(inarg.args[0].value as *const fuse_release_in) };
            let mut reply = ReplyEmptyInternal { reply: Err(-(ENOSYS as i32)) };
            fs.releasedir(sb, &req, inarg.h.nodeid, release_in.fh, release_in.flags,
                         &mut reply);
            match reply.reply() {
                Ok(_) => {
                    0
        ...
        fuse_opcode_FUSE_FSYNCDIR => {
            ...
            fs.fsyncdir(sb, &req, inarg.h.nodeid, fsync_in.fh, datasync,
                       &mut reply);
            match reply.reply() {
                Ok(_) => {
                    0
        ...
        fuse_opcode_FUSE_SETXATTR => {
            ...
            let mut reply = ReplyEmptyInternal { reply: Err(-(ENOSYS as i32)) };
            fs.setxattr(sb, &req, inarg.h.nodeid, name, value, setxattr_in.flags, setxattr_in.size, &mut reply);
            match reply.reply() {
                Ok(_) => {
                    0
                },
        ...
        fuse_opcode_FUSE_CREATE => {
            ...
            fs.create(sb, &req, inarg.h.nodeid, name, create_in.mode, create_in.flags, &mut reply);
            match reply.reply() {
                Ok(_) => {
                    0
                },
        ...
        fuse_opcode_FUSE_GETLK => {
            ...
            fs.getlk(sb, &req, inarg.h.nodeid, getlk_in.fh, getlk_in.owner, getlk_in.lk.start,
        ...
        fuse_opcode_FUSE_SETLK => {
            ...
            let mut reply = ReplyEmptyInternal { reply: Err(-(ENOSYS as i32)) };
            fs.setlk(sb, &req, inarg.h.nodeid, setlk_in.fh, setlk_in.owner,
        ...
        fuse_opcode_FUSE_BMAP => {
            ...
            fs.bmap(sb, &req, inarg.h.nodeid, bmap_in.blocksize, bmap_in.block, &mut reply);
            match reply.reply() {
                Ok(_) => {
                    0
                },
