...
impl BentoFileSystem<'_> for Xv6fsFileSystem {
    ...
    fn bento_init(
        ...
        let foo_msg = message.init_root::<foo::Builder>();
        foo_msg.set_msg("hello from xv6fs client");
        serialize::write_message(&mut steram, &message);
    ...
    fn bento_destroy(&mut self, _req: &Request) {
        let msg = format!("exit {}", Ipv4Addr::LOCALHOST);
    ...
    fn bento_statfs(&self, _req: &Request, _ino: u64, reply: ReplyStatfs) {
        let msg = format!("statfs {}", Ipv4Addr::LOCALHOST);
    ...
    fn bento_open(
        ...
        let msg = format!("open {} {} {}", Ipv4Addr::LOCALHOST, nodeid, flags);
    ...
    fn bento_opendir(
        ...
        let msg = format!("opendir {} {}", Ipv4Addr::LOCALHOST, nodeid);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
    ...
    fn bento_getattr(&self, _req: &Request, nodeid: u64, reply: ReplyAttr) {
        let msg = format!("getattr {} {}", Ipv4Addr::LOCALHOST, nodeid);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
    ...
    fn bento_setattr(
        ...
        let msg = format!("setattr {} {} {}", Ipv4Addr::LOCALHOST, ino, size);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
    ...
    fn bento_create(
        ...
        let msg = format!("create {} {} {}", Ipv4Addr::LOCALHOST, parent, name_str);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
    ...
    fn bento_mkdir(
        ...
        let name_str = name.to_str().unwrap();
        let msg = format!("mkdir {} {} {}", Ipv4Addr::LOCALHOST, parent, name_str);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
    ...
    fn bento_lookup(
        ...
        let name_str = name.to_str().unwrap();
        let msg = format!("lookup {} {} {}", Ipv4Addr::LOCALHOST, nodeid, name_str);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
    ...
    fn bento_read(
        ...
        let msg = format!("read {} {} {}", Ipv4Addr::LOCALHOST, nodeid, offset, size);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut read_bytes = 0;
        let mut read_off = 0;
        let mut bento_resp = [0 as u8; size];
        // TODO: might not need this loop
        while read_bytes < size {
            let mut msg_resp = [0 as u8; 4096];
            let read_size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
                Ok(x) => x,
                Err(_) => {
                    reply.error(libc::EIO);
                    return;
            ...
            match *read_vec.get(0).unwrap() {
                "Ok" => {
                    bento_resp[read_off..read_size] = &msg_resp[3..read_size];
        ...
        reply.data(bento_resp);
        
    }

    // TOOD: might need to add loop to write until data is written to server
    fn bento_write(
        &self,
        _req: &Request,
        nodeid: u64,
        ...
        let msg = format!("write {} {} {} {}", Ipv4Addr::LOCALHOST, nodeid, offset, str::from_utf8(data).unwrap());
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
    ...
    fn bento_readdir(
        ...
        let msg = format!("readdir {} {} {}", Ipv4Addr::LOCALHOST, nodeid, offset);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
    ...
    fn bento_rmdir(&self, _req: &Request, parent: u64, name: &OsStr, reply: ReplyEmpty) {
        let name_str = name.to_str().unwrap();
        let msg = format!("rmdir {} {} {}", Ipv4Addr::LOCALHOST, parent, name_str);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
    ...
    fn bento_unlink(&self, _req: &Request, parent: u64, name: &OsStr, reply: ReplyEmpty) {
        let name_str = name.to_str().unwrap();
        let msg = format!("unlink {} {} {}", Ipv4Addr::LOCALHOST, parent, name_str);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
    ...
    fn bento_fsync(
        ...
        let msg = format!("fsync {}", Ipv4Addr::LOCALHOST);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
    ...
    fn bento_fsyncdir(
        ...
        let msg = format!("fsyncdir {}", Ipv4Addr::LOCALHOST);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
    ...
    fn bento_symlink(
        ...
        let msg = format!("symlink {} {} {} {}", Ipv4Addr::LOCALHOST, nodeid, name_str, linkname_str);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
    ...
    fn bento_readlink(&self, _req: &Request, nodeid: u64, reply: ReplyData) {
        let msg = format!("readlink {} {}", Ipv4Addr::LOCALHOST, nodeid);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
    ...
    fn bento_rename(
        ...
        let msg = format!("fsyncdir {} {} {} {} {} {}", Ipv4Addr::LOCALHOST, parent_ino, name_str, newparent_ino, newname_str, flags);
        let _ = self.socket.as_ref().unwrap().write(msg.as_bytes());
        let mut msg_resp = [0 as u8; 4096];
        let size = match self.socket.as_ref().unwrap().read(&mut msg_resp) {
            Ok(x) => x,
